...
router.get('', async (req, res, next) => {
  try {
    ...
    _.map(smart_result, (e, i) => {
      ...
      var x = e.dataValues.x;
      var y = e.dataValues.y;

      if (x === 0 || y === 0) {
        var orthogonal = busan_Rectangle_range(
          e.dataValues.latlon.coordinates[1],
          e.dataValues.latlon.coordinates[0],
        );
        e.dataValues.x = orthogonal[1];
        e.dataValues.y = orthogonal[0];
      }
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    if (x === 0 || y === 0) {
      var orthogonal = busan_Rectangle_range(
        smart_one.latlon.coordinates[1],
        smart_one.latlon.coordinates[0],
      );
      smart_one.x = orthogonal[1];
      smart_one.y = orthogonal[0];
    }
