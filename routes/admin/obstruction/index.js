...
router.get('', async (req, res) => {
  try {
    ...
    sequelize.ObsRelation.belongsTo(sequelize.ObsCompany, {
      foreignKey: 'company_idx',
    });
    sequelize.ObsRelation.belongsTo(sequelize.ObsModel, {
    ...
    if (req.query.area_id !== undefined) {
      var fids =
        req.query.area_id === undefined || null
          ? ''
          : Config.createEncoding(req.query.area_id);
        ...
        attributes: [
          ...
          [
            sequelize.sequelize.fn(
              'X',
              sequelize.sequelize.col('obstruction_TBL.latlon'),
            ),
            'lat',
          ],
          [
          ...
          [
            sequelize.sequelize.fn(
              'date_format',
              sequelize.sequelize.col('obstruction_TBL.measure_date'),
              '%Y-%m-%d %H:%i:%s',
            ),
            'measure_date',
          ],
          ...
          'hole_depth',
          [
            sequelize.sequelize.col('obstruction_model_relation_TBL.code_idx'),
            'code_idx',
          ],
          [
            sequelize.sequelize.col(
              'obstruction_model_relation_TBL.manhole_idx',
            ),
            'manhole_idx',
          ],
          [
            sequelize.sequelize.col(
              'obstruction_model_relation_TBL->obstruction_manhole_model_TBL.width',
            ),
            'width',
          ],
          [
            sequelize.sequelize.col(
              'obstruction_model_relation_TBL->obstruction_manhole_model_TBL.diameter',
            ),
            'diameter',
          ],
          [
            sequelize.sequelize.col(
              'obstruction_model_relation_TBL->obstruction_manhole_model_TBL.height',
            ),
            'height',
          ],
          [
            sequelize.sequelize.col(
              'obstruction_model_relation_TBL->obs_relation_TBL->obs_company_TBL.company_name',
            ),
            'company_name',
          ],
          [
            sequelize.sequelize.col(
              'obstruction_model_relation_TBL->obs_relation_TBL->obs_model_TBL.model_name',
            ),
            'model_name',
          ],
        ],
  ...
  try {
    var obstruction_one = await sequelize.Obstruction.findOne({
      attributes: [
        ...
        [
          sequelize.sequelize.col('obstruction_model_relation_TBL.manhole_idx'),
          'manhole_idx',
        ],
        [
          sequelize.sequelize.col(
    ...
    var orthogonal = busan_Rectangle_range(
      obstruction_one.latlon.coordinates[1],
      obstruction_one.latlon.coordinates[0],
    );

    obstruction_one.x = orthogonal[1];
    obstruction_one.y = orthogonal[0];
...
router.post('', async (req, res, next) => {
  ...
  try {
    ...
    let point = undefined;
    if (!x === false && x > 0 && !y === false && y > 0) {
      let wgs84 = busan_Rectangle_WGS84(y, x);
      point = { type: 'Point', coordinates: [wgs84[1], wgs84[0]] };
    } else if (!lat === false && lat > 0 && !lon === false && lon > 0) {
      let ract = busan_Rectangle_range(req.body.lon, req.body.lat);
      x = ract[1];
      y = ract[0];
      point = { type: 'Point', coordinates: [req.body.lat, req.body.lon] };
    }
    ...
    var filePath =
      process.platform === 'linux'
        ? `/efs/dtx_image/obs_images/${find_area.area_id}/`
        : path.join(
            __dirname.substr(0, __dirname.length - 25),
            `public/${find_area.area_id}/`,
          );
    // base64 decoding
      ...
      case '6':
        ...
        var code = await sequelize.ObsRelation.findOne({
          attributes: [
            'idx',
            [
              sequelize.sequelize.col('obs_company_TBL.company_name'),
              'company_name',
            ],
                ...
                return Promise.all(promise).then((promise_result) => {
                  return sequelize.Obstruction.findOne({
                    attributes: [
                      ...
                      [
                        sequelize.sequelize.col(
                          'obstruction_model_relation_TBL.code_idx',
                        ),
                        'code_idx',
                      ],
                      [
...
router.delete('/:id', async (req, res, next) => {
  ...
  try {
    ...
    // modeling delete
    shelljs.exec(
      `sudo rm -rf /efs/dtx_model_web/tilesets/data/${areaOne.real_id.toLocaleUpperCase()}/hole/b3dm/high/${
        req.params.id
      }`,
    );
    shelljs.exec(
      `sudo rm -rf /efs/dtx_model_web/tilesets/data/${areaOne.real_id.toLocaleUpperCase()}/hole/b3dm/low/${
        req.params.id
      }`,
    );
    shelljs.exec(
      `sudo rm -rf /efs/dtx_model_web/tilesets/data/${areaOne.real_id.toLocaleUpperCase()}/hole/lod/${
        req.params.id
      }`,
    );
    shelljs.exec(
      `sudo rm -rf /efs/dtx_model_web/tilesets/data/${areaOne.real_id.toLocaleUpperCase()}/hole/obj/${
        req.params.id
      }`,
    );
