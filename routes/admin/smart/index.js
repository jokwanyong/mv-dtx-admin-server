      ...
      attributes: [
        ...
        [
          sequelize.sequelize.fn(
            'X',
            sequelize.sequelize.col('smart_station_TBL.latlon'),
          ),
          'lat',
        ],
        [
        ...
        'material',
        [
          sequelize.sequelize.col('pipe_model_relation_TBL.smart_model'),
          'smart_model',
        ],
        [
      ...
      where: {
        [Op.and]: [
          ...
          sequelize.sequelize.where(
            sequelize.sequelize.fn(
              'date',
              sequelize.sequelize.col('smart_station_TBL.measure_date'),
            ),
            '2020-05-22',
          ),
        ],
        ...
        .then((findA) => {
          ...
          var filePath =
            process.platform === 'linux'
              ? `/efs/dtx_image/gps_images/${findA.area_id}/`
              : path.join(
                  __dirname.substr(0, __dirname.length - 18),
                  `public/${findA.area_id}/`,
                );
          ...
          return sequelize.Smart.create(
            ...
            return sequelize.ModelRelation.create(
              ...
              insert_smart.dataValues.lat =
                insert_smart.dataValues.latlon.coordinates[0].toFixed(12);
              insert_smart.dataValues.lon =
                insert_smart.dataValues.latlon.coordinates[1].toFixed(12);
...
router.delete('/:id', async (req, res, next) => {
  ...
  try {
    ...
    shelljs.exec(
      `sudo rm -rf /efs/dtx_model_web/tilesets/data/${areaOne.real_id.toLocaleUpperCase()}/joint/b3dm/high/${
        req.params.id
      }`,
    );
    shelljs.exec(
      `sudo rm -rf /efs/dtx_model_web/tilesets/data/${areaOne.real_id.toLocaleUpperCase()}/joint/b3dm/low/${
        req.params.id
      }`,
    );
    shelljs.exec(
      `sudo rm -rf /efs/dtx_model_web/tilesets/data/${areaOne.real_id.toLocaleUpperCase()}/joint/lod/${
        req.params.id
      }`,
    );
    shelljs.exec(
      `sudo rm -rf /efs/dtx_model_web/tilesets/data/${areaOne.real_id.toLocaleUpperCase()}/joint/obj/${
        req.params.id
      }`,
    );
